...
export default function JobCard({ job, isHost = false, onEdit, onDelete, onViewCandidates }) {
  ...
  const getStatusColor = (status) => {
    const statusColors = {
      ...
      closed: 'bg-yellow-100 text-yellow-700 border-yellow-300',
  ...
  const handleDelete = async (e) => {
    ...
    if (onDelete) {
      ...
      let message = `Are you sure you want to ${action} this job?`;
      if (hasApplications) {
        message += `\n\nThis job has ${job.currentApplications} application(s). Cancelling will:\n• Stop accepting new applications\n• Notify all applied candidates\n• Preserve application data for review\n\nThis action cannot be undone.`;
      } else {
      ...
      if (window.confirm(message)) {
        try {
          ...
          if (response.ok) {
            ...
            const successMessage = result.action === 'cancelled'
              ? 'Job has been cancelled successfully. Candidates have been notified.'
  ...
  return (
    <div className="bg-white rounded-lg shadow-md border border-gray-200 hover:shadow-lg transition-shadow duration-200 overflow-hidden">
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          {isHost && (
            <div className="flex items-center gap-1 ml-4">
              ...
              {/* Always allow edit for host unless job is completed */}
              ...
              {/* Always allow delete/cancel for host */}
              <button
                onClick={handleDelete}
                className="p-2 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                title={job.currentApplications > 0 ? "Cancel Job" : "Delete Job"}
                disabled={loading}
              >
        ...
        {/* Interview Progress for Host View */}
        {isHost && job.status === 'interviews_active' && (
          <div className="mb-4 p-3 bg-purple-50 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-purple-700">Interviews</span>
              <span className="text-sm text-purple-600">
                {job.completedInterviews || 0} completed
              </span>
            </div>
            {job.maxCandidatesShortlist && (
              <div className="flex items-center justify-between text-xs text-purple-500">
                <span>
                  {job.maxCandidatesShortlist - (job.completedInterviews || 0)} pending
                </span>
                <span>Target: {job.maxCandidatesShortlist}</span>
              </div>
            )}
          </div>
        )}
