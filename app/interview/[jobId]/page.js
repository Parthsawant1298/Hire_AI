...
export default function VoiceInterviewPage() {
  ...
  const [voiceVerificationStatus, setVoiceVerificationStatus] = useState('pending');
  ...
  const isVerifyingVoiceRef = useRef(false);
  const lastVoiceVerificationTime = useRef(0);
  const callDurationRef = useRef(0);
  const voiceVerificationInterval = 30000; // Verify voice every 30 seconds
  ...
  useEffect(() => {
    ...
    const initDetector = async () => {
      try {
        setMpStatus('📦 Initializing AI...');
        
        // Load MediaPipe vision module via dynamic import (it's an ES module)
        let FilesetResolver, FaceDetector;

        if (window._mpVision) {
          ({ FilesetResolver, FaceDetector } = window._mpVision);
        } else {
          const cdns = [
          ...
          for (const url of cdns) {
            try {
              vision = await import(/* webpackIgnore: true */ url);
              break;
            } catch (e) {
              console.warn(`Failed to load vision module from ${url}`, e);
            }
          }
          if (!vision) throw new Error('Failed to load MediaPipe vision module from all CDNs');

          ({ FilesetResolver, FaceDetector } = vision);
          window._mpVision = { FilesetResolver, FaceDetector };
        }
        ...
        setMpStatus('✅ AI Ready');

        const detectFrame = async () => {
          if (!detector || !videoRef.current || !canvasRef.current || !videoOn) {
          ...
          if (detections.length > 0) {
            ...
            // Draw Box
            ctx.strokeStyle = boundingBoxColorRef.current;
            ctx.lineWidth = 4;
            ...
            ctx.fillText(verificationUserNameRef.current || 'Candidate', bbox.originX + 5, bbox.originY - 8);

            // Periodically verify against backend
            const now = Date.now();
            if (now - lastVerificationTime.current > verificationInterval && !isVerifyingRef.current) {
        ...
        setMpStatus('❌ AI Error');
        setError(`AI Init Failed: ${err.message}`);
      }
    };

    const verifyFace = async () => {
      if (!videoRef.current || !userRef.current?.verificationFaceImage) return;
      ...
      try {
        ...
        const res = await fetch('/api/interview/verify-face', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            stored_image_url: userRef.current.verificationFaceImage,
        ...
        if (data.verified) {
          ...
          setVerificationUserName(userRef.current.name || 'Verified');
        } else {
          setVerificationStatus('failed');
          setBoundingBoxColor('#ef4444');
          setVerificationUserName('Unknown');
        }
      } catch (e) {
  ...
  }, [videoOn, user]);  // Re-run if user/status changes

  // Real-time Voice Verification Loop
  useEffect(() => {
    let voiceIntervalId;

    if (isCallActive && permissionGranted) {
      voiceIntervalId = setInterval(async () => {
        const now = Date.now();
        if (now - lastVoiceVerificationTime.current > voiceVerificationInterval && !isVerifyingVoiceRef.current) {
          await captureAndVerifyVoice();
          lastVoiceVerificationTime.current = now;
        }
      }, 5000); // Check every 5 seconds if interval has passed
    }
  ...
  const captureAndVerifyVoice = async () => {
    if (isVerifyingVoiceRef.current) return;

    try {
      isVerifyingVoiceRef.current = true;
      console.log('🎤 Capturing real-time voice snippet for verification...');
      ...
      mediaRecorder.onstop = async () => {
        ...
        // Stop the verification stream tracks immediately to not keep mic active
        stream.getTracks().forEach(track => track.stop());

        if (audioBlob.size < 2000) {
          console.log('🎤 Voice snippet too short, skipping verification');
          isVerifyingVoiceRef.current = false;
          return;
        }

        const formData = new FormData();
        formData.append('testAudio', audioBlob, 'realtime-voice.webm');
        ...
        try {
          ...
          console.log('🎵 Real-time voice verification result:', result);

          if (result.verified) {
            setVoiceVerificationStatus('verified');
          } else {
            setVoiceVerificationStatus('failed');
            // Log anomaly to transcript if similarity is very low
            if (result.ensembleScore < 0.4) {
              console.warn('🚩 Voice identity mismatch detected!');
              // You could add an anomaly entry to monitoringData here
            }
          }
        } catch (err) {
      ...
      console.error('❌ Failed to capture voice snippet:', err);
      isVerifyingVoiceRef.current = false;
    }
  };
  ...
  const checkVerificationStatus = async () => {
    try {
      ...
      // 1. Check URL for immediate verification pass (from pre-interview page)
      const urlVerified = searchParams.get('verified') === 'true';
      if (urlVerified) {
        console.log('✅ Verification passed via URL parameter');
      ...
      // 2. Check if verification was already passed in this session
      ...
      // 3. Check user's verification setup status (baseline data)
  ...
  const checkPreviousInterview = async (userId) => {
    if (!userId) return;
    try {
      // Backend uses token to find user, but we pass userId for explicit mapping
      const response = await fetch(`/api/jobs/${params.jobId}/application-status`, {
        credentials: 'include'
      });
          ...
          <div className="space-y-3">
            <button
              onClick={() => {
                ...
                router.push(`/verification/pre-interview?jobId=${params.jobId}&assistant=${assistant || ''}`);
              }}
