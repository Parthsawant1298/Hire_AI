...
export async function GET(request, { params }) {
  try {
    ...
    if (isInterviewAccess) {
      ...
      if (!application) {
        console.log(`❌ No application found for user ${user._id} on job ${jobId}`);
        return NextResponse.json(
          { 
            error: 'Application not found', 
            message: 'You have not applied to this job. Please apply before attempting the interview.',
      ...
      const allowedStatuses = ['shortlisted', 'interview_scheduled', 'interview_completed', 'ai_selected'];
      if (!allowedStatuses.includes(application.status)) {
        console.log(`❌ User ${user._id} has status '${application.status}' for job ${jobId}`);
        return NextResponse.json(
          { 
            error: 'Not authorized for interview',
            message: `Your application status is '${application.status}'. Only shortlisted candidates can take the interview.`,
