...
export async function POST(request, { params }) {
  try {
    ...
    if (selectedCandidateIds.length > job.positionsAvailable) {
      return NextResponse.json(
        { error: `Cannot select more than ${job.positionsAvailable} candidates` },
        { status: 400 }
      );
    }
    ...
    // Update rejected candidates (those not selected)
    ...
    for (const application of selectedApplications) {
      application.status = 'hr_selected';
    ...
    for (const application of rejectedApplications) {
      application.status = 'hr_rejected';
      await application.save();
