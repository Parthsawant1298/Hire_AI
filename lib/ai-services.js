...
export async function analyzeResume({ resumeText, jobDescription, jobRequirements, jobTitle }) {
  try {
    ...
    console.log('🤖 Analyzing resume with REAL AI (OpenRouter/GPT-4o-mini)...');
...
export async function analyzeVoiceInterview({
  ...
  try {
    ...
    // **REAL AI ANALYSIS** - Send transcript to OpenAI for actual evaluation
    console.log('🤖 Sending transcript to OpenAI for real analysis...');
...
`;
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert HR interviewer and talent evaluator. Analyze interview transcripts honestly and critically. Give low scores for poor performance, high scores for excellent performance.'
        },
        {
          role: 'user',
          content: prompt
    ...
    const analysis = {
      ...
      processingMethod: 'real-ai-analysis-openai'
    };
    ...
    console.error('Self-contained interview analysis error:', error);
    ...
    return {
      ...
      detailedFeedback: 'Interview completed successfully. Analysis processed locally without external dependencies.',
      keyPoints: ['Interview participation', 'Basic communication demonstrated'],
      areasForImprovement: ['Continue developing skills', 'Practice interview techniques'],
      metrics: {
        completionRate: 100,
        averageResponseLength: 50,
        interviewDuration: Math.round((interviewDuration || 600) / 60),
        questionsAnswered: answeredQuestions || 1,
